...
class CminiControllerClass {
  ...
  getLayoutById(hash: string) {
    ...
    const boardLayouts = layout?.boardIds.map(
      (boardId) => CminiStore.boardLayouts.get(boardId)!,
    );
    return {
      boardLayouts,
      layout,
  ...
  getMetas(metaIds: string[]) {
    return metaIds
      .map((hash) => CminiStore.meta.get(hash))
      .filter((hash) => !!hash);
  }

  getBoardLayoutsByAuthorId(authorId: string) {
    const boardIds =
      authorId in CminiStore.indexToMany.authorIdToBoardIds
        ? CminiStore.indexToMany.authorIdToBoardIds[authorId]
        : [];
