...
export default function Keyboard({
  ...
  useEffect(() => {
    ...
    for (const key of keys) {
      ...
      ref[key.column] = {
        ...
        heatColor: gradientValue(
          heatmap[String(key.key)],
          theme.heatmap.cold,
          theme.heatmap.hot,
        ),
  ...
  const toggleMap = () =>
    display === "heatmap" ? setDisplay("fingermap") : setDisplay("heatmap");
    ...
    <Stack className={style["keyboard"]}>
      <Stack className="keys">
        {keyboard.map((columns, i) => (
          <div
            className={style["key-row"]}
            key={i}
            style={{
              marginLeft: shouldStagger
                ? (config?.size || 50) *
                  (Math.floor(i / 2) * 0.5 +
                    Math.floor(i / 2) * 0.33 +
                    (i % 2 > 0 ? 0.33 : 0))
          ...
          >
            {columns.map(({ key, heatColor, fingerColor, heat, finger }) => (
              <div
                className={style["key-container"]}
                key={key}
                style={{ width: config.size || 50, height: config.size || 50 }}
              >
                <div
                  className={style["key"]}
                  style={{
                    backgroundColor: shouldShowHeatmap
                      ? heatColor
                      : fingerColor,
                    color: theme.text,
